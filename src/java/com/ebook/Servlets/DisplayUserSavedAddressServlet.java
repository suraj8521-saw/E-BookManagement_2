 ...
 * @author suraj
 */
public class DisplayUserSavedAddressServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            System.out.println(bookAddToCardId);
            String userEmail=request.getParameter("userEmail");
            out.print(userEmail);
            
            UserAddressSaveDataOperation addressData=new UserAddressSaveDataOperation(ConnectionProvider.getConnection());
            List<UserAddressSave>address=addressData.getUserAddressByEmail(userEmail);
             String content;
            String color;
            String icon;
            Message mymessage;
...
//                session1.setAttribute("bookData", book);
               ...
               List<UserAddressSave> addressList = addressData.getUserAddressByEmail(userEmail);
session7.setAttribute("addressList", addressList);
if(bookAddToCardId!=null && !bookAddToCardId.trim().isEmpty()){
session8.setAttribute("bookAddToCardId",Integer.valueOf(bookAddToCardId) );
}

                response.sendRedirect("User/UserSavedAddressDisplay.jsp");
            }
           else{
                    content="No Saved Address Found Please Add a New Address ";
                color="red";
                icon="fa fa-times";
                mymessage=new Message(content, color, icon);
                session.setAttribute("mymessage", mymessage);
